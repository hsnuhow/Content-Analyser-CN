...
| `content-crawler` | 爬取引擎 → 純文字 | 4Gi / cpu 2 / timeout 300 / concurrency 4 | undetected-chromedriver · Chrome · Selenium |
...
**回滾方式（兩種）：**
1. **Cloud Run revision 回滾**（最快，不動程式碼）：
   ```bash
   gcloud run revisions list --service <svc> --region asia-east1
   gcloud run services update-traffic <svc> --to-revisions <健康revision>=100 --region asia-east1
   ```
