...
embeddings/{key}                  embedding 內容快取（key=sha256(model:dim:text)）
  vector(float[]) / model / dim / created_at
  （向量化前先查、miss 才算 Vertex 再寫回；key 含 model+dim → 換模型自動失效。fail-safe）
...
# 附錄 F：產品術語與使用流程標準（規範，2026-06-19 訂）

> 單一真實來源。所有 UI 文案、新功能、文件一律依此；一個詞只有一個意思，禁止同義異字。
...
## F.2 術語表（每個詞唯一定義，括號內為「禁用的混用字」）

| 正式用詞 | 定義 |
|---------|------|
| **內容來源** | 把內容弄進專案的 3 種入口的傘狀分類 |
| **關鍵字推薦** | search-extent 內容發現；產物＝**推薦清單**（禁用：自動推薦爬取清單）|
| **推薦清單** | 關鍵字推薦的結果，暫存可勾選（禁用：推薦筆記）|
| **貼上網址** | 貼 URL 建立草稿資料集的入口 |
| **上傳資料** | Cowork／外部蒐集，直接建立「完成」資料集的入口（上傳 .txt/.md/.docx 檔／貼上文字／進階 JSON）|
| **資料集** | 可分析的內容單位（唯一核心名詞；禁用：清單／網址清單）|
| **資料集狀態** | `草稿` → `爬取中` → `完成`（手動匯入直接＝完成）|
| **分析** | 對一或多個資料集跑引擎的過程（禁用：一鍵分析／送去分析，統一動詞用「**送出分析**」）|
| **合併分析** | 勾選多個資料集一起分析（保留，因語意明確）|
| **報告** | 分析的產物：Markdown ＋ 數值 CSV |
| **延伸報告** | 報告下游、各專家的行動指引 |
...
|------|---------|
| 關鍵字推薦產生 | 「產生推薦清單」 |
| 推薦清單 → 草稿 | 「建立草稿資料集」／「併入現有草稿」 |
| 貼網址建立 | 「建立草稿資料集」 |
| 草稿送爬 | 「開始爬取」 |
| 資料集送分析 | 「送出分析」（多選時「合併分析」）|

## F.4 頁面分區標準（專案頁）

1. **內容來源**（一區，三入口：關鍵字推薦／貼上網址／上傳資料）
2. **資料集**（統一列表＋狀態徽章；推薦清單收於此區上方）
3. **分析與報告**（送出分析〔單/多〕＋歷史報告）
4. 側欄：成員／專案管理

> 編號只用於「主線四階段」，並列入口不各自編號（杜絕 ⓪①② 與無編號混排）。
