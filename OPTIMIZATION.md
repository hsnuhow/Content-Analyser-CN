...
**✅ 已清**：crawler 漏洞審查 3 項（🟠 High SSRF-Chrome / 🟡 ReDoS / 🟡 Cloud Tasks 金鑰落地）、list_projects N+1（member_emails 索引）、Token 全面記帳行為驗證、並行爬蟲佇列實爬驗證（並行+無 OOM）、巨檔模組化（nlp_path、project_routes 2366→package、crawler.py 抽 page_classify+text_clean）、CSP `script-src` unsafe-inline 移除、JSON 清理去重、測試環境機制 + rollback.sh。

**⏸️ 目前刻意暫緩（低風險維護性，已評估、有案可查）**：

| 項目 | 風險 | 暫緩原因 |
|------|------|---------|
| crawler.py DOM 內容評分群拆分 | 低 | 核心抽取品質邏輯、深度互呼叫，簡單頁爬取無法驗品質回歸（見 [crawler 模組化記憶]）|
