...
| 項目 | 嚴重度 | 說明 / 建議 | 狀態 |
|------|--------|------------|------|
| crawler SSRF redirect 殘留 | High | 入口 `_is_safe_url` 已解析 DNS 收口；但 og/oEmbed/YouTube 的 urllib 抓取與 Chrome 導航**跟隨 redirect 時未重驗** → 公開 URL 302 到內網仍可能。需 redirect handler 逐跳重驗 / egress proxy | ✅ **已修**：urllib 路徑 `net_guard.safe_urlopen` 逐跳重驗；Chrome `driver.get` 路徑 2026-06-21 補 `is_safe_ip`+perf-log 查 remote IP 丟棄。根治見 [docs/ssrf-posture.md](docs/ssrf-posture.md) |
| crawler 同步 /api/scrape 無看門狗 | High | async 批次（crawl_job）有看門狗；同步單篇路徑步驟內 hang 無上限。應比照包 `ThreadPoolExecutor.result(timeout)` | ✅ **已修**（2026-06-21）：`_tier1_scrape` 包 `ThreadPoolExecutor + result(timeout=min(hard_timeout+30, 290))`，留在 Cloud Run 300s 內 |
| crawler 回收/recycle close() 可能卡住 | High | 看門狗砍掉 hung thread 靠 `old.close()`；若 close 本身 hang 會卡整批。應對 close 加超時 | ✅ **已修**（2026-06-21）：app.py 新增 `_force_close()`，close() 包 15s 上限、逾時 kill Chrome 進程（對齊 crawl_job） |
| content-analyser list_projects/list_all_users 全集合掃描 | Medium | 每次列表掃整個 collection 查 member；應加 `member_emails` array 欄位 + `array_contains` 查詢 + 既有專案 backfill | ✅ **已修**（2026-06-21）：projects 加 `member_emails` 陣列 + `array_contains` 雙索引查詢（admin 仍全掃）；既有專案以 `/admin/backfill-member-emails` 補欄位 |
| 分析平行階段缺取消檢查點 | Low | 兩路平行（最貴 LLM 段）期間無 `cancel_requested` 檢查；取消後仍跑完才停 | 部分緩解（security_audit #7 附加式 should_stop）|
...
## 2026-06-21 清理進度與目前刻意暫緩項

本日大批清理（每項實跑驗證、見 [changelog.md](changelog.md)），backlog 現況：

**✅ 已清**：crawler 漏洞審查 3 項（🟠 High SSRF-Chrome / 🟡 ReDoS / 🟡 Cloud Tasks 金鑰落地）、list_projects N+1（member_emails 索引）、Token 全面記帳行為驗證、並行爬蟲佇列實爬驗證（並行+無 OOM）、**crawler.py 抽取層三層化（2569→1423 行 -45%；page_classify/text_clean/dom_score/dom_parse/site_templates/dom_extract 六模組，Porsche before/after 逐刀零回歸）**、project_routes 2366→package、nlp_path、CSP `script-src` unsafe-inline 移除、JSON 清理去重、測試環境機制 + rollback.sh。

**⏸️ 目前刻意暫緩（低風險維護性，已評估、有案可查）**：

| 項目 | 風險 | 暫緩原因 |
|------|------|---------|
| term_filters 三層巢狀 N+1 | 低 | 已加 `MAX_SCAN_DATASETS` 上限緩解；根治需改資料結構 |
| analysis 每日配額非原子 | 低 | 軟限制，高並行下理論可超量些微 |
| CSP `style-src 'unsafe-inline'` | 低（XSS）| inline 樣式屬性多、移除工程大、XSS 風險低 |
| O-2 輪詢成本 / O-3 result_markdown 1MB | 中/低 | 規模成長後才痛 |
