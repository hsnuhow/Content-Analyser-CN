...
- **Phase 2b 待辦**：爬蟲選擇器輔助（`research._ask_gemini_for_selector`，系統 GENAI）尚未計帳——它在爬蟲服務深層流程、需另行 thread db/usage，暫緩避免動到選擇器學習。
- 驗證：三服務 `py_compile` + `bash -n` 通過；`norm_usage`/`aggregate`/`write_system_usage`/`_record_usage` 單元 + mock 整合測試通過。分支 `feat/token-accounting`（worktree），未部署。
