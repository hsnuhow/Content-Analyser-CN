...
- **補清理**：爬蟲 /api/crawl/cleanup 從「只清 crawl_jobs」擴成清 3 個 job 集合（crawl + image_extract + research）——之前已完成的圖片擷取/研究記錄（純文字網址）永遠累積、不會被清；現一併逾 N 天刪。admin 既有「清除孤兒/陳舊任務」按鈕即涵蓋。
- 不做（A 下無檔案）：下載存檔/容量上限/壓縮。
