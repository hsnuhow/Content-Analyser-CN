...
def _is_valid_selector(selector: str) -> bool:
    ...
    if s in _TOO_BROAD_SELECTORS:
        return False
    # 含 3+ 連續數字的 id（如 #post-12345 / #container94110）→ 單篇專屬、不可泛化
    if _DIGIT_ID.search(s):
        return False
    return True


# ── 選擇器正規化（P1：耐用性）──
# 寫入 learned_selectors 前正規化，讓選擇器能泛化到同網域其他文章、不因改版失效。
_PASCAL_HASH = re.compile(r'^([A-Z][A-Za-z0-9]*)-([A-Za-z0-9]{5,})$')  # styled-components：PascalCase-雜湊
_ATOMIC_CLASS = re.compile(r'^(?:css|sc|emotion)-[A-Za-z0-9]{4,}$', re.I)  # emotion/sc 原子類（無語意名）
_DIGIT_ID = re.compile(r'#[A-Za-z_][\w-]*\d{3,}')                      # 含 3+ 連續數字的 id（單篇專屬）
_NTH_PSEUDO = re.compile(r':nth-(?:child|of-type)\([^)]*\)')           # 脆弱位置偽類


def _looks_hashy(s: str) -> bool:
    """判斷字串是否像建構期雜湊：含數字，或大小寫混雜（一般英文單字全小寫，不會命中）。"""
    return bool(re.search(r'\d', s) or (re.search(r'[a-z]', s) and re.search(r'[A-Z]', s)))


def normalize_selector(selector: str) -> str:
    """正規化選擇器以利泛化。回正規化後字串；判定不可泛化（純原子類/數字 id）→ 回空字串。
    規則：
      - `.ComponentName-aB3dEf`（styled-components PascalCase-雜湊）→ `[class*="ComponentName"]`
      - `.css-1q2w3e` / `.sc-bdVaJa`（框架原子類，無語意）→ 不可泛化 → ""
      - `#post-12345`（含 3+ 連續數字 id）→ 不可泛化 → ""
      - `:nth-child(...)` / `:nth-of-type(...)` → 剝除（脆弱位置）
      - 乾淨穩定 class（`.entry-content`）、語意標籤（`article`）→ 原樣保留
    """
    s = _NTH_PSEUDO.sub("", (selector or "").strip()).strip()
    if not s:
        return ""
    if _DIGIT_ID.search(s):
        return ""
    classes = re.findall(r'\.([A-Za-z_][\w-]*)', s)
    if any(_ATOMIC_CLASS.match(c) for c in classes):
        return ""  # 含純框架原子類 → 不可泛化

    def _repl(m):
        cls = m.group(1)
        hm = _PASCAL_HASH.match(cls)
        if hm and _looks_hashy(hm.group(2)):
            return f'[class*="{hm.group(1)}"]'
        return "." + cls

    s = re.sub(r'\.([A-Za-z_][\w-]*)', _repl, s)
...
def save_learned_selector(domain: str, selector: str,
    ...
    if not domain or not selector:
        return
    # P1：寫入前正規化（雜湊 class→前綴、剝 nth、拒原子類/數字 id）。讀取端仍有 ≥300字/非列表/非cookie 兜底。
    norm = normalize_selector(selector)
    if norm and norm != selector:
        print(f"[SiteLearning] 正規化選擇器：{selector!r} → {norm!r}", flush=True)
    selector = norm
    if not selector or not _is_valid_selector(selector):
        print(f"[SiteLearning] 選擇器不可泛化/過寬，拒絕寫入 {domain}：{selector!r}", flush=True)
        return
    try:
        from firebase_admin import firestore
