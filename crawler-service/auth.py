# -*- coding: utf-8 -*-
"""
金鑰驗證（content-crawler）

驗證順序：
  1. 系統金鑰（Secret Manager 注入的 CRAWLER_API_KEY）— 常數時間比對，向後相容。
  2. api_keys 白名單（content-analyser 核發，存 Firestore api_keys collection）—
     比對 SHA-256 hash、檢查 is_active 與 permissions 是否含所需權限。
...
def is_authorized(provided_key: str, system_key: str,
    ...
    """驗證 X-API-Key。db 為 firestore client（可為 None，則只比對系統金鑰）。"""
    # 1. 系統金鑰（向後相容）
